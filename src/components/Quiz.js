...
function Quiz() {
      ...
      <div className="d-flex justify-content-center align-items-center vh-100">
        <div className="card shadow-lg p-4 text-start w-50" style={{ background: "linear-gradient(135deg, #e9f9f6 0%, #e9f9f6 100%)" }}>
          ...
          <div className="form-check mb-3">
            <input type="checkbox" className="form-check-input me-2" id="agree"
  ...
  return (
    <div className="vh-100 vw-100 p-3 d-flex" style={{ background: "linear-gradient(135deg, #6a85b6 0%, #bac8e0 100%)" }}>
      ...
      {/* Left Palette */}
      <div className="d-flex flex-column flex-shrink-0 me-3" style={{ width: "200px" }}>
        <div className="card p-3 shadow-sm bg-white flex-grow-1 d-flex flex-column">
          ...
          <div className="d-grid gap-2 flex-grow-1">
            {quiz.questions.map((_, index) => {
              ...
              return (
                <button key={index} className={btnClass} onClick={() => setCurrentQ(index)}>
                  {index + 1}
                </button>
              );
            })}
      ...
      <div className="flex-grow-1 d-flex align-items-center justify-content-center">
        <div className="card p-4 shadow bg-light text-dark flex-grow-1" style={{ maxHeight: "100%", overflowY: "auto" }}>
          <h5 className="mb-4">{q.question}</h5>
          {q.options.map((option, i) => (
            <div key={i} className="form-check mb-2">
              <input className="form-check-input" type="radio" id={`q${currentQ}-opt${i}`} name={`q-${currentQ}`}
                value={i} checked={answers[currentQ] === i} onChange={() => handleOptionChange(currentQ, i)} />
          ...
          <div className="d-flex justify-content-between mt-4">
            <button className="btn btn-outline-secondary" disabled={currentQ === 0} onClick={handlePrev}>
              Previous
            </button>
            <button className="btn btn-primary" onClick={handleNext} disabled={answers[currentQ] === null}>
